...
#define BLE_CHIP_TYPE             BLE_CHIP_HC05
...
#define XLIGHT_BLE_ROLE		        '1'		          // 0=>Slave role; 1=>Master role; 2=>Loopback
#else
#define XLIGHT_BLE_BAUD           '4'             // 1=>1200 baudios, 2=>2400, 3=>4800, 4=>9600 (default), 5=>19200, 6=>38400, 7=>57600, 8=>115200
#define XLIGHT_BLE_ROLE		        'M'		          // S=>Slave role; M=>Master role
#endif
...
BLEInterfaceClass::BLEInterfaceClass()
{
  ...
  m_bATMode = false;
  m_name = XLIGHT_BLE_SSID;
  m_pin = XLIGHT_BLE_PIN;
  m_lastCmd = "";
}
...
void BLEInterfaceClass::setName(String _name)
{
  if( _name != m_name) {
    m_name = _name;
    ...
    digitalWrite(m_pin_en, HIGH);
    // Adjusting bluetooth name
    BLEPort.printlnf("AT+NAME=%s", m_name.c_str());
    // Stop configuration
    digitalWrite(m_pin_en, LOW);
#else
    // Adjusting bluetooth name
...
void BLEInterfaceClass::setPin(String _pin)
{
  if( _pin != m_pin) {
    m_pin = _pin;
    ...
    digitalWrite(m_pin_en, HIGH);
    // Password adjust
    BLEPort.printlnf("AT+PSWD=%s", m_pin.c_str());
    // Stop configuration
    digitalWrite(m_pin_en, LOW);
#else
    // Password adjust
